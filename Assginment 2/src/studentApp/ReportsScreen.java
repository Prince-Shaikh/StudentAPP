...
public class ReportsScreen extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals("A")) {
			...
			if(index != 0) {
				Courses course = App.courses.get(index-1);
			...
			int index = comboA.getSelectedIndex();
			if(index != 0) {
				Student student = App.students.get(index-1);
				
				Courses[] courses = new Courses[App.courses.size()];
				...
				for(int j = 0; j<courses.length; j++) {
					...
					if(courses[j].getEnrolledStudents().length != 0) {
						...
						Student[] s = courses[j].getEnrolledStudents();
