...
public class StudentScreen extends JFrame implements ActionListener{
	...
	private JComboBox comboBoxSemster;
	...
	public StudentScreen() {
		...
		lblStudentInformation.setFont(new Font("Tahoma", Font.BOLD, 26));
		lblStudentInformation.setBounds(50, 13, 370, 50);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals("Close")) {
			...
			App.students.add(new Student(name, fatherName, gender, cms, semster));
			...
			String input = ""; 
			input = JOptionPane.showInputDialog(null, "Enter Students CMS");

			
			if(input.equals("") != true) {
				int cms = Integer.parseInt(input);
				
				int index = searchStudent(cms);
				if(index != -1) {
					fillForm(index);
	...
	private void fillForm(int index) {
		Student student = App.students.get(index);
	...
	private int searchStudent(int cms) {
		for(int i = 0; i<App.students.size();i++) {
			if(App.students.get(i).getCMS()==cms) {
				return i;
			}
		}
		return -1;
